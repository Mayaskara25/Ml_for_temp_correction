...
class SensorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("MAX6675 Sensor System Manager")
        ...
        self.sensor_ok_var = tk.StringVar(value="--")
    ...
    def build_ui(self):
        ...
        ttk.Label(self.root, text="  MAX6675 SENSOR MANAGER", style="Header.TLabel").pack(anchor="w", padx=10)
        ...
        ttk.Label(tree_frame, text="  |-- MAX6675 Sensor Status   : ").grid(row=2, column=0, sticky="w")
        ttk.Label(tree_frame, textvariable=self.sensor_ok_var, style="Value.TLabel").grid(row=2, column=1, sticky="w")

        ttk.Label(tree_frame, text="\n[-] Simulated / Model Values").grid(row=3, column=0, sticky="w")
        ttk.Label(tree_frame, text="  |-- Synthetic PT100 Ref     : ").grid(row=4, column=0, sticky="w")
        ttk.Label(tree_frame, textvariable=self.pt100_var, style="Value.TLabel").grid(row=4, column=1, sticky="w")
        ttk.Label(tree_frame, text="  |-- Synthetic K-Type Input  : ").grid(row=5, column=0, sticky="w")
        ttk.Label(tree_frame, textvariable=self.synthetic_ktype_var, style="Value.TLabel").grid(row=5, column=1, sticky="w")
    ...
    def update_ui_safe(self, status, pt100, ktype, synthetic_ktype, corrected, error, sensor_ok):
        if status:
            self.status_var.set(status)
        if pt100:
            self.pt100_var.set(pt100)
        if ktype:
        ...
        if sensor_ok is not None:
            self.sensor_ok_var.set(sensor_ok)

    def read_data(self):
        try:
            ser = serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=1)
            self.root.after(0, self.update_ui_safe, f"[+] Hardware Status: CONNECTED ({SERIAL_PORT})", None, None, None, None, None, None)
                ...
                if ser.in_waiting > 0:
                    ...
                    if len(parts) == 6:
                        try:
                            ...
                            pt_val = f"{float(parts[3]):.2f} °C"
                            corr_val = f"{float(parts[4]):.2f} °C"
                            err_val = f"{(float(parts[4]) - float(parts[3])):+.2f} °C"
                            sensor_ok = "OK" if parts[5].strip() == "1" else "ERROR"
                            self.root.after(0, self.update_ui_safe, None, pt_val, live_k_val, synthetic_k_val, corr_val, err_val, sensor_ok)
                            last_data_time = time.time()
                        except ValueError:
                            pass
            ...
            self.root.after(0, self.update_ui_safe, "[+] Hardware Status: DISCONNECTED (Simulation Mode)", None, None, None, None, None, "N/A")
            base_temp = 25.0
            ...
            while self.running:
                ...
                self.root.after(0, self.update_ui_safe, None, pt_val, k_val, synthetic_k_val, corr_val, err_val, "SIM")
                time.sleep(0.5)
